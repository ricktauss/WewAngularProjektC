...
export class ContractSearchComponent implements OnInit {
  ...
  search():void {
    ...
    this.contractService.search(this.firstName, this.lastName, this.baseURL).subscribe({
  ...
  update(): void {
    ...
    this.contractService.update(this.baseURL, this.selectedContract!).subscribe({
      next: (contract) => {
